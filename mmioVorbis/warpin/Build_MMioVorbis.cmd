/*
        ...
        MMioVorbis.WPI, containing the MMioVorbis program
        (C) VOICE 
*/
...
major=VALUE('MMIOFLACMAJOR',,'OS2ENVIRONMENT')
minor=VALUE('MMIOFLACMINOR',,'OS2ENVIRONMENT')
revision=VALUE('MMIOFLACREVISION',,'OS2ENVIRONMENT')
fixlevel=VALUE('MMIOFLACFIXLEVEL',,'OS2ENVIRONMENT')
...
DO UNTIL (lines(InName)=0)
  linex=linein(InName)
  If POS('PACKAGEID="Netlabs\MMioVorbis',linex)>0 then Do
    linex=linex||version||'"'
  end
...
/* Package 1 MMioVorbis */
warpindir"\wic.exe "pkgname||fileversion" -a 1 -r -cMMioVorbis * -s "||pkgname||fileversion||".wis"
