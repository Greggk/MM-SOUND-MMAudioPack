/*
        ...
        MMioMP3.WPI, containing the MMioMP3 program
        (C) VOICE 
*/
...
major=VALUE('MMioMP3MAJOR',,'OS2ENVIRONMENT')
minor=VALUE('MMIOMP3MINOR',,'OS2ENVIRONMENT')
revision=VALUE('MMioMP3REVISION',,'OS2ENVIRONMENT')
fixlevel=VALUE('MMioMP3FIXLEVEL',,'OS2ENVIRONMENT')
...
DO UNTIL (lines(InName)=0)
  linex=linein(InName)
  If POS('PACKAGEID="Netlabs\MMioMP3',linex)>0 then Do
    linex=linex||version||'"'
  end
...
/* Package 1 MMioMP3 */
warpindir"\wic.exe "pkgname||fileversion" -a 1 -r -cMMioMP3 * -s "||pkgname||fileversion||".wis"
